...
const logger = require('./utils/logger') // import archivo logger
...
app.use((req, res, next) => {
  ...
  if (login && login == '123') {
    // http://localhost:3010/api/v1/comentarios?login=123
    next()
  } else {
...
app.listen(port, async () => {
  ...
  logger.access.info(`Server levantado en http://localhost:${port}`)
  try {
