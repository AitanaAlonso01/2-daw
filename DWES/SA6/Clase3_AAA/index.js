...
//Configurar la sesion de express
app.use(
  session({
    name: 'sid', // nombre de la cookie de sesion (opcional)
    secret: process.env.SESSION_SECRET || 'clave_super_secreta', // para firmar la cookie con clave privada
    resave: false, //no queremos que se guarde la sesión si esta no cambia
    saveUninitialized: false, //para no crear sesiones vacías
    cookie: {
      httpOnly: true, //desde js no podamos acceder a la cookie
      secure: false, //HTTP --> secure:true si es https
